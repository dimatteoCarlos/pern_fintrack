...
// 🎯 OTHER DELETION METHODS - ENGLISH
  ...
  closeOnlyBlockedNotice: "This account still holds a balance, so it cannot be closed yet. Bring it to zero with ordinary movements and then close it, which keeps its name on every movement that mentions it. The other route is the button below: it adjusts the accounts this one has moved money with and then removes this one WITHOUT keeping its name.",
...
// 🎯 OTHER DELETION METHODS - SPANISH
  ...
  closeOnlyBlockedNotice: "Esta cuenta todavía tiene saldo, así que aún no se puede cerrar. Llévelo a cero con movimientos normales y ciérrela: su nombre queda en cada movimiento que la menciona. La otra vía es el botón de abajo: ajusta las cuentas con las que esta movió dinero y después la elimina SIN conservar su nombre.",
