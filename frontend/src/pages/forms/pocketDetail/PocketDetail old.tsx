...
import { AccountListType, PocketListType } from '../../../types/responseApiTypes.ts';
...
//data initializing
const initialPocketDetail = DEFAULT_POCKET_LIST[0]
...
function PocketDetail() {
  ...
  const {pocketData, previousRoute}:LocationStateType = location.state as LocationStateType | null;

  const {pocketId}=useParams()
  const previousRouteFromState = state?.previousRoute ?? "/";

  console.log(pocketData, previousRoute)
//------------------------
