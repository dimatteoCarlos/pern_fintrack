...
- Investigate why the session expires even when a refresh token exists and should have been updated. PENDIENTE — **the mechanism was corrected on 2026-09-09** (`e7804cdd`): the refresh cookie was written with no `maxAge` and no `expires` (`backend/src/utils/authUtils/cookieConfig.js:10-15`), so it was a session cookie that died on browser close while the token inside stayed valid; it now carries a 7-day `maxAge`, the same lifetime the signature and the row declare. **Still open on the human check:** close the browser, reopen it, and confirm the session is now kept.
- Define how to keep the user signed in while the refresh token remains valid. PENDIENTE — **no longer blocked by the cookie**, which has carried a 7-day lifetime since `e7804cdd`; what remains is the product decision on how long "remembered" should mean.
...
## Backend and Security — 6 LISTO / 2 PENDIENTE
...
- Organize cookie and token duration rules. **LISTO 2026-09-09** (`e7804cdd`) — three sources declared three lifetimes for the same refresh token: the JWT was signed for 8.9 days (`authFn.js:95-98`), the database row expired at 7 (`authController.js:149`, `:287`, `authFn.js:180`), and the cookie declared none at all (`cookieConfig.js:8-23`), which made it a session cookie that died on browser close. `REFRESH_TOKEN_DAYS` in `backend/src/utils/authUtils/authFn.js` is now the only place that number is written, and the signature, the row and the cookie all read it; the cookie gained a `maxAge`. Seven was chosen because it is what the row already enforced — the refresh endpoint filters `expiration_date > NOW()` (`authRefreshToken.js:42`) — so no live session is shortened. The three response comments that said "60 minutos" in one endpoint and "15 minutos" in two others about the same 3600 were corrected; the value itself was right and the client never reads it back, since `tokenExpiry` is written (`useAuth.ts:277-279`) and only ever cleared (`invalidateSession.ts:34`, `logoutCleanup.ts:37`). Code and lesson in `FIXES-LOG.md`.
- **The refresh-token rotation threshold carried a factor of one thousand too many, so the token rotated on every refresh.** **LISTO 2026-09-09** (`e7804cdd`) — the total lifetime is already in milliseconds (`authRefreshToken.js:81`) and the threshold multiplied it by a thousand again (`:86`), putting it a thousand lifetimes ahead of a remainder that never exceeds one, so the comparison at `:91` was always true and the declared 10%-remaining-life rule never applied. Each rotation revokes one row and inserts another (`authFn.js:164-194`), so `refresh_tokens` grew by one row per refresh call, unbounded. The threshold now stands at 16.8 hours of a 7-day life. Measured while reading the item above; it was in no list.
...
## Account editor register — 5 LISTO / 6 PENDIENTE
...
- **Three account types receive no currency from the read endpoint (E-11).** 🟡 Media. **LISTO 2026-09-09 — not reproducible, and nothing was changed.** The half of the entry about the write endpoint still holds: bank, investment and income-source accounts have identical, empty editable surfaces, because that endpoint has no case for them and they are absent from its table map, which is correct. The other half does not. The file this entry cites holds no currency reference in any of its 393 lines, and the account read endpoint joins `currencies` live in every branch: bank and investment (`backend/src/fintrack_api/controllers/getAccountController.js:319`), category budget (`:338`), pocket (`:399`), debtor (`:427`). No commented-out currency join exists on any account read; the only one in `backend/src` sits in a dashboard query (`dashboardController.js:704`). Either the join was restored without this entry being updated, or the entry was wrong when written — the git history was not searched to tell which, because the outcome is the same. **Closed as measured, not as fixed.**
...
## Locale and money formatting — 1 LISTO / 3 PENDIENTE

- **The money formatter's default currency cannot be found in the currency catalog.** **LISTO 2026-09-09** (`4a840346`) — **and the fix was not where this entry pointed.** The upper-case `'USD'` at `frontend/src/fintrack/helpers/functions.ts:39` reaches only case-insensitive consumers: `currencyFormat` hands the code to `Intl.NumberFormat`, which ignores case in an ISO code; `getCurrencySymbol` upper-cases both sides before comparing (`functions.ts:74`); `isValidCurrencyCode` upper-cases its input against an upper-case set (`functions.ts:257`). No call site indexes `CURRENCY_OPTIONS` with either default, so that literal produces no `undefined`. **The reachable form of the same symptom was one file away:** the default currency is read from the environment and cast to `CurrencyType` without being normalised (`frontend/src/fintrack/helpers/currencyConstants.ts:73-74`), and the cast is what hides a bad value from the compiler, because `CurrencyType` is a lower-case-only union (`frontend/src/fintrack/types/types.ts:214`). An operator who sets `VITE_ACCOUNTING_CURRENCY_CODE=USD` — the natural way to write a currency code — leaves `CURRENCY_OPTIONS[DEFAULT_CURRENCY]` `undefined` at all 23 sites that index it, and every formatter falls back in silence to the machine's locale. The value is now lower-cased and checked against `SUPPORTED_CURRENCIES`, with `'usd'` as the fallback, so an unsupported code such as `gbp` is caught by the same guard. The variable is set in no `.env` today, so nothing changed locally. Code and lesson in `FIXES-LOG.md`.
...
**125 items — 85 LISTO, 40 PENDIENTE.** Per-group counts are in
