...
export const updateAffectedAccountBalance = async (
  ...
  if (!dbClient || !dbClient.query) {
    throw createError(
      500,
      'updateAffectedAccountBalance requires an active database client (transactional context).',
    );
  }
  ...
  try {
    console.log(
      pc.blue(
        `Updating balance for affected account ${accountId} to ${newBalance}`,
      ),
    );
...
`,
    ...
    const updateAffectedAccountResult = await dbClient.query(
      updateAffectedAccountBalanceQuery,
    );

    if (updateAffectedAccountResult.rows.length === 0) {
      throw createError(
        404,
        `Account ${accountId} not found for balance update.`,
      );
    }
